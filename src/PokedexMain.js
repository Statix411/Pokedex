...
const PokedexMain = observer(() => {
  ...
  if (pokemonClassPerems.oneName) {
    let data = fetch(
      `https://pokeapi-215911.firebaseapp.com/api/v2/pokemon/${
        pokemonClassPerems.oneName
      }`
    )
      .then(response => response.json())
      .then(responseJson => {
        let abilitys = responseJson.abilities.map(num => (
          <li key={num.ability.name}>{num.ability.name}</li>
        ))
        let types = responseJson.types.map(num => (
          <li key={num.type.name}>{num.type.name}</li>
        ))
        ...
        ReactDOM.render(
          <div className='PokedexBack' style={sty}>
            <div className='Super'>
              ...
              <ul className='abilitie'>{abilitys}</ul>
              <div className='Types'>Types: </div>
              <ul className='typesul'>{types}</ul>
            </div>
            <div
              className='PokedexButton1'
              onClick={e => {
          ...
          document.getElementById('PokedexMain')
        )
      })
      .catch(error => {
        console.error(error)
